...
import { Loader2, ChevronDown, AlertTriangle, ClipboardCopy } from "lucide-react";
...
interface JourneyReleaseFormProps {
...
export function JourneyReleaseForm({ item, onClose, onSaved }: JourneyReleaseFormProps) {
  ...
  // Editable fields - pre-populated from checklist data
  ...
  const [dtLiberacao, setDtLiberacao] = useState<string | null>(item.dtLiberacao || nowISO());
  ...
  const validateAndConfirm = () => {
    ...
    if (!dtLiberacao) {
      toast({ title: "Campo obrigatório", description: "Informe a Data de Liberação.", variant: "error" });
      return;
    }
    setConfirmOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      dtCheckList: item.dtCheckList || null,
      dtLiberacao: dtLiberacao || null,
    ...
    try {
      ...
      toast({ title: "Sucesso", description: "Liberação salva com sucesso." });
      onSaved();
    } catch (err) {
  ...
  return (
    <>
      <FloatingPanel title="Liberação de Viagem" onClose={onClose} width={920}>
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-4 gap-3">
              ...
              <div className="space-y-1">
                <div className="min-h-[22px] flex items-center">
                  <Label className="text-xs">Data de Liberação <span className="text-destructive">*</span></Label>
                </div>
                <DatePickerField
                  value={dtLiberacao}
                  onChange={setDtLiberacao}
                  includeTime
                  hasError={!dtLiberacao}
                  disabled={isAlreadyReleased}
                />
              </div>
            </div>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-border">
            ...
            {!isAlreadyReleased && (
              <Button size="sm" className="h-8 text-xs gap-1" onClick={validateAndConfirm} disabled={saving}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              Confirmar Liberação de Viagem
            </AlertDialogTitle>
            <AlertDialogDescription>
              <span className="block mb-2">
                Ao confirmar a liberação, os dados desta viagem <strong>não poderão mais ser editados</strong>.
              </span>
              <span className="block text-xs text-muted-foreground">
                Motorista: <strong>{motoristaLiberado}</strong> · Veículo: <strong>{veiculoLiberado}</strong>
              </span>
              <span className="block mt-2 font-medium text-foreground">
                Deseja realmente liberar esta viagem?
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="text-xs">Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleSave} className="text-xs bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Sim, Liberar Viagem
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
